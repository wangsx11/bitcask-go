...
func decodeMedtadata(buf []byte) *metadata {
	dataType := buf[0]
	var index = 1
...
func (hk *hashInternalKey) enocde() []byte {
...
func (hk *hashInternalKey) encodePrefix() []byte {
	buf := make([]byte, 8+len(hk.key))
	var index = 0
	// key
	copy(buf[index:index+len(hk.key)], hk.key)
	index += len(hk.key)
...
func decodeHashInternalKey(encKey []byte, keyLen int) *hashInternalKey {
	var index = 0
	// key
	key := encKey[index : index+keyLen]
	index += keyLen
	// version
	version := int64(binary.LittleEndian.Uint64(encKey[index : index+8]))
	index += 8
	field := encKey[index:]
...
func (sk *setInternalKey) enocde() []byte {
	buf := make([]byte, 8+len(sk.key)+len(sk.mumber))
	// key
	var index = 0
